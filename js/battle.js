...
const Battle = (() => {
  ...
  // -------------------------------------------------------- Kern-Logik ---
  // Reine, isomorphe Gen-1-Mathe (Typentabelle, Stats, EXP, Schaden, Fangen,
  // Evolutionen, seedbare RNG) lebt in js/battle-core.js — eine Quelle der
  // Wahrheit für Solo UND den künftigen PvP-Server (siehe MULTIPLAYER_PLAN.md).
  const BC = (typeof BattleCore !== 'undefined') ? BattleCore
           : (typeof require !== 'undefined' ? require('./battle-core.js') : null);
  const { CHART, SPECIAL, EVO, effectiveness, calcStats, expFor, damage, catchChance } = BC;
